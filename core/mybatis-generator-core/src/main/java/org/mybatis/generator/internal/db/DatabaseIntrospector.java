...
public class DatabaseIntrospector {
    ...
    public List<IntrospectedTable> introspectTables(TableConfiguration tc)
        ...
        //youzhihao:设置均衡字段标记
        initBalanceColumnsFlag(tc, columns);
    ...
    //youzhihao:设置均衡字段标记
    private void initBalanceColumnsFlag(TableConfiguration tc, Map<ActualTableName, List<IntrospectedColumn>> columns) {
        for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns.entrySet()) {
            Iterator<IntrospectedColumn> tableColumns = entry.getValue().iterator();
            while (tableColumns.hasNext()) {
                IntrospectedColumn introspectedColumn = tableColumns.next();
                if (tc.isColumnBalance(introspectedColumn.getActualColumnName())) {
                    introspectedColumn.setBalance(true);
                }
            }
        }
    }
